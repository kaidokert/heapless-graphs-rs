...
mod tests {
    ...
    use crate::adjacency_list::{map_adjacency_list::MapAdjacencyList, slice_adjacency_list::SliceAdjacencyList};
    use crate::matrix::{simple_matrix::Matrix, bit_map_matrix::BitMapMatrix};
    ...
    where
        NI: Default + PartialEq + Copy + core::fmt::Debug + SliceIndex<[bool], Output = bool> + PartialOrd + 'a,
        E: Graph<NI>,
    ...
    fn test_basic_traversals() {
        ...
        let map_adj_list = MapAdjacencyList::new(FnvIndexMap::<_, _, 8>::from_iter(
            [
                (1_usize, NodeValueStructOption([Some((5, 'x')), None, Some((4, 'a'))])),
    ...
    fn test_dfs_fix_verification() {
        ...
        for i in 0..result.len() {
            for j in i+1..result.len() {
                assert_ne!(result[i], result[j], 
                    "DFS produced duplicate node {}: {:?}", result[i], result);
            }
        }
        ...
        assert_eq!(result.len(), 4, "Should visit exactly 4 nodes, got: {:?}", result);
    ...
    fn test_cross_representation_dfs_consistency() {
        ...
        dfs_iterative_unchecked(&edge_list_graph, 0, visited1.as_mut_slice(), stack1, &mut |x| {
            collector1.push(x);
        }).unwrap();
        ...
        dfs_iterative_unchecked(&edge_node_graph, 0, visited2.as_mut_slice(), stack2, &mut |x| {
            collector2.push(x);
        }).unwrap();
        ...
        for result in [collector1.result(), collector2.result(), collector3.result()] {
