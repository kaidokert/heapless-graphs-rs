...
trait ProtoGraph {
    ...
    fn neighboring_nodes(&self, node: Self::MyNodeIndex) -> impl Iterator<Item = Self::MyNodeIndex>;
}
...
mod tests {
    ...
    fn test_edges() {
        ...
        assert_eq!(collected_slice, &[(0, 0), (0, 1), (1, 0), (1, 1), (1, 2), (2, 0), (2, 2)]);
    }
    ...
    fn test_outgoing_edges() {
        ...
        assert_eq!(&[0, 1], collect_to_mut_slice(pen_matrix.outgoing_edges(0), &mut collect));
        assert_eq!(&[0, 1, 2], collect_to_mut_slice(pen_matrix.outgoing_edges(1), &mut collect));
        assert_eq!(&[0, 2], collect_to_mut_slice(pen_matrix.outgoing_edges(2), &mut collect));
    }
    ...
    fn test_incoming_edges() {
        let pen_matrix = super::PenMatrix::<8, _, _, _>::new(make_test_matrix());
        let mut collect = [0usize; 16];
        assert_eq!(&[0, 1,2], collect_to_mut_slice(pen_matrix.incoming_edges(0), &mut collect));
        assert_eq!(&[0, 1], collect_to_mut_slice(pen_matrix.incoming_edges(1), &mut collect));
        assert_eq!(&[1, 2], collect_to_mut_slice(pen_matrix.incoming_edges(2), &mut collect));
    }
    #[test]
    fn test_neighboring_nodes() {
        let pen_matrix = super::PenMatrix::<8, _, _, _>::new(make_test_matrix());
        let mut collect = [0usize; 16];
        assert_eq!(&[0, 1, 2], collect_to_mut_slice(pen_matrix.neighboring_nodes(0), &mut collect));
        assert_eq!(&[0, 1, 2], collect_to_mut_slice(pen_matrix.neighboring_nodes(1), &mut collect));
        assert_eq!(&[0, 1, 2], collect_to_mut_slice(pen_matrix.neighboring_nodes(2), &mut collect));
    }
}
