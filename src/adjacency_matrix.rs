...
where
    NI: NodeIndex + SliceIndex<[Option<EV>]>,
...
where
    NI: NodeIndex + SliceIndex<[Option<EV>]> + core::convert::From<usize>,
...
where
    NI: NodeIndex + core::slice::SliceIndex<[Option<EV>]> + core::convert::From<usize>,
    INNER: AsRef<[Option<EV>]>,
    T: AsRef<[INNER]>,
{
    type Error = GraphError<NI>;

    type Edges<'a> = WeirdIter<'a, NI, EV, T, INNER>
    where
        Self: 'a,
        NI: 'a;

    type Nodes<'a> = core::slice::Iter<'a, NI>
    where
        Self: 'a,
        NI: 'a;
